...
print("training_transforms variable")
training_transforms = transforms.Compose([#Microscope(),
                                        ...
                                        transforms.Normalize([0.485, 0.456, 0.406], 
                                                            [0.229, 0.224, 0.225])]) 
print("testing_transforms variable")
...
def get_parameters(net, EXCLUDE_LIST) -> List[np.ndarray]:
        ...
        print("get_parameters() start - utils.py")
        parameters = []
        ...
        print("get_parameters() end - utils.py")
        return parameters


def set_parameters(net, parameters, EXCLUDE_LIST):

        print("set_parameters() start - utils.py")
        keys = []
        ...
        print("set_parameters() end! - utils.py")


class Net(nn.Module):
    def __init__(self, arch, return_feats=False):
        # print("class Net init")
    ...
    def forward(self, images):
        # print("class Net's forward() starts")
        """
        ...
        if self.return_feats:
            return features
        # print("class Net's forward() ends")
        return output
...
def train(model, train_loader, validate_loader, num_examples,partition, nowandb, device="cuda",  log_interval = 100, epochs = 10, es_patience = 3):
    ...
    #torch.save(best_model.state_dict(), "./model/model.pth")
